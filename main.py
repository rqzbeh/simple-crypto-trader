...
# Small sentinel class used as placeholder for optional modules for the static analyzer.
# When an optional module import fails, the real symbol will override this, but
# having this sentinel defined prevents static analysis from thinking the name
# might be undefined or that a None value might be called.
...
# Predefine optional module-level symbols to avoid static analyzer "possibly unbound" errors
ProbabilityPredictor = _MissingOptional
MultiProviderLLMClient = _MissingOptional
EnsembleSignalGenerator = _MissingOptional
SymbolStrategyManager = _MissingOptional
Backtester = _MissingOptional
TradeMonitor = _MissingOptional
...
try:
    ...
    ML_AVAILABLE = False
    print(
        "Warning: ML libraries not available. Install scikit-learn, numpy, pandas, joblib"
    )
...
if not NEWS_API_KEY:
    logger.warning(
        "NEWS_API_KEY not set. Running in demo news mode (no live articles)."
    )
...
probability_predictor = (
    ProbabilityPredictor(llm_client=llm_client)
    if ML_ENABLED and predictor_available
    else None
)
...
if LLM_AVAILABLE and llm_client and hasattr(llm_client, "get_budget_status"):
    # Show budget status
    budget_status = llm_client.get_budget_status()  # type: ignore[attr-defined]
...
ensemble_generator = (
    EnsembleSignalGenerator(market_analyzer) if ENSEMBLE_AVAILABLE else None
)
...
def fetch_rss_feed(url, timeout=10):
    ...
    try:
        resp = requests.get(
            url, timeout=timeout, headers={"User-Agent": "CryptoTrader/1.0"}
        )
        items = []
...
def get_crypto_news():
    ...
    if NEWS_API_KEY and newsapi:
        ...
        articles = [
            {
                "title": "Bitcoin institutional accumulation continues",
                "description": "Major funds increase BTC positions amid macro uncertainty",
                "source": "DEMO",
                "publishedAt": now,
            },
            {
                "title": "Ethereum upgrade successful",
                "description": "Network efficiency improves, gas costs reduced significantly",
                "source": "DEMO",
                "publishedAt": now,
            },
            {
                "title": "Solana ecosystem expands rapidly",
                "description": "New dApps launch, trading volume hits new highs",
                "source": "DEMO",
                "publishedAt": now,
            },
            {
                "title": "XRP gains regulatory clarity",
                "description": "Court ruling provides positive outlook for cross-border payments",
                "source": "DEMO",
                "publishedAt": now,
            },
            {
                "title": "Cardano development milestone reached",
                "description": "Smart contract upgrades roll out successfully",
                "source": "DEMO",
                "publishedAt": now,
            },
            {
                "title": "BNB utility expands across chains",
                "description": "New partnerships announced for DeFi integration",
                "source": "DEMO",
                "publishedAt": now,
            },
            {
                "title": "Avalanche sees increased developer activity",
                "description": "Subnet deployments surge as ecosystem grows",
                "source": "DEMO",
                "publishedAt": now,
            },
            {
                "title": "Dogecoin merchant adoption rises",
                "description": "Payment integrations expand to major retailers",
                "source": "DEMO",
                "publishedAt": now,
            },
        ]
    ...
    print(
        f"Fetched {len(articles)} news articles ({'demo' if not NEWS_API_KEY else 'live'})"
    )
    return articles
...
def analyze_sentiment_with_llm(articles, symbol=""):
    ...
    if cached_articles:
        print(
            f"[CACHE] Using {len(cached_articles)} cached AI analyses (saving API calls)"
        )
    ...
    if new_articles:
        print(
            f"[AI] Analyzing {len(new_articles)} new articles with Multi-Provider LLM"
        )
...
REASON: [your 2-3 sentence explanation]"""
        ...
        try:
            ...
            if hasattr(llm_client, "chat"):
                result = llm_client.chat(  # type: ignore[attr-defined]
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a cryptocurrency market sentiment analyzer. Analyze news and provide sentiment scores between -1.0 (very bearish) and +1.0 (very bullish). If your model outputs values in the -100..+100 range, the signal generator will automatically normalize them to [-1.0, +1.0].",
                        },
                        {"role": "user", "content": prompt},
                    ]
                )
            else:
                # The LLM client doesn't expose a `chat` method (e.g. an optional sentinel),
                # so treat it as a failure for this analysis and skip further processing.
                result = None
            ...
            # Normalize LLM score: some providers return values in [-100, 100] while others use [-1, 1].
            # If the parsed score lies outside the [-1, 1] range, it is assumed to be on the [-100, 100]
            # scale and is normalized accordingly to [-1, 1].
            raw_score = score
            if abs(raw_score) > 1.0:
                try:
                    normalized = float(raw_score) / 100.0
                    logger.debug(
                        f"[AI] Normalizing LLM raw score {raw_score} -> {normalized}"
                    )
                    score = normalized
                except Exception:
                    # If normalization fails, log and keep the original value so the clamp below still applies
                    logger.debug(
                        f"[AI] Failed to normalize LLM raw score {raw_score}, using original value"
                    )

            # Clamp score to [-1.0, 1.0] to be safe
    ...
    if all_scores:
        ...
        try:
            ...
            print(
                f"[SOCIAL] News sentiment: {news_sentiment:.3f}, Social sentiment: {social_sentiment:.3f}"
            )
...
def safe_last(series):
    """Return the last element of a pandas Series or numpy array robustly."""
    try:
        # Pandas Series/Index has iloc
        if hasattr(series, "iloc"):
            return series.iloc[-1]
        # fallback: use .values or convert to numpy array
...
def get_market_data(symbol, period="30d", interval="1h"):
    ...
    try:
        ...
        close = df["Close"]
        # Convert to pandas Series if needed so that .pct_change() and similar work reliably
        if not hasattr(close, "pct_change"):
            close = pd.Series(np.asarray(close))
        last_close_val = safe_last(close)
...
def calculate_trade_signal(
    sentiment_score, news_count, market_data, symbol="", news_articles=None
):
    """
    ...
    adaptive_params = (
        market_analyzer.get_adjusted_parameters()
        if market_analyzer
        else {
    ...
    for indicator, weight in INDICATOR_WEIGHTS.items():
        if indicator in indicators and "signal" in indicators[indicator]:
            ...
            weight_multiplier = (
                market_analyzer.get_indicator_weight_multiplier(indicator)
                if market_analyzer
                else 1.0
            )
    ...
    combined = (
        market_analyzer.combine_analyses(
            tech_score_normalized, sentiment_score, llm_analysis
        )
        if market_analyzer
        else {
    ...
    if probability_predictor and combined.get("ml_probability") is None:
        ...
        ml_prob = None
        if probability_predictor and hasattr(probability_predictor, "get_probability"):
            ml_prob = probability_predictor.get_probability(prelim_signal)  # type: ignore[attr-defined]
        if ml_prob is not None:
            combined["ml_probability"] = ml_prob
            combined["confidence"] = (
                combined["confidence"] * ML_CONF_BLEND_HEURISTIC_WEIGHT
            ) + (ml_prob * ML_CONF_BLEND_MODEL_WEIGHT)
    ...
    base_leverage = min(
        math.floor(
            rr_ratio + (confidence_factor * 5)
        ),  # Add up to 5x based on confidence
    ...
    if probability_predictor and combined.get("ml_probability") is not None:
        ...
        if ml_prob_final < 0.5:
            ...
            leverage = min(
                adaptive_params.get("dynamic_max_leverage", MAX_LEVERAGE_CRYPTO),
                leverage + ML_LEVERAGE_HIGH_PROB_INCREMENT,
            )
...
def log_trade(symbol, signal, sentiment_reason="", indicators_data=None):
    ...
    try:
        ...
        logger.debug(f"Could not read {TRADE_LOG_FILE}: {e}")
        logs = []
    ...
    try:
        with open(DAILY_RISK_FILE, "r") as f:
            ...
            if risk_data.get("date") == today:
                ...
                try:
                    if market_analyzer:
                        adaptive_limit = market_analyzer.get_adjusted_parameters().get(
                            "daily_risk_limit"
                        )
                except Exception:
                    adaptive_limit = None
                limit_to_use = (
                    adaptive_limit if adaptive_limit is not None else DAILY_RISK_LIMIT
                )
                return risk_data.get("loss_pct", 0) >= limit_to_use
    except Exception as e:
        logger.debug(f"Could not read {DAILY_RISK_FILE}: {e}")
    return False
...
def check_trade_outcomes():
        ...
        try:
            ...
            if intraday_data.empty:
                ...
                trade_window = intraday_data[
                    (intraday_data.index >= entry_time) & (intraday_data.index <= now)
                ]
                ...
                if trade_window.empty:
                    ...
                    if current_data.empty:
                        ...
                        last_close_val = safe_last(current_data["Close"])
                        if last_close_val is None:
                            continue
                        current_price = float(last_close_val)
                    ...
                    last_close_val = safe_last(trade_window["Close"])
                    if last_close_val is None:
                        continue
                    current_price = float(last_close_val)
    ...
    if updated:
        ...
        if queued_count > 0:
            print(
                f"[⏳] {queued_count} trades still in queue (waiting for entry or completion)"
            )
    ...
    def smart_format_price(price, reference_prices=None):
        ...
        for decimals in [6, 8, 10, 12]:
            ...
            if reference_prices:
                ...
                ref_formatted = [
                    f"{ref:.{decimals}f}".rstrip("0").rstrip(".")
...
📈 Technical: {signal["technical_score"]:.2f}"""
    ...
    try:
        ...
        elif probability_predictor and hasattr(
            probability_predictor, "get_pending_status"
        ):
            pending_info = probability_predictor.get_pending_status()  # type: ignore[attr-defined]
            if pending_info:
                msg += f"\n🧪 ML/AI: {pending_info}"
    except Exception:
        pass
    ...
    try:
        ...
        try:
            with open(TRADE_LOG_FILE, "r") as f:
                logs = json.load(f)
            open_trades = len(
                [t for t in logs if t.get("status") in ["open", "queued"]]
            )
            completed_trades = len(
                [t for t in logs if t.get("status") not in ["open", "queued"]]
            )
            ...
            logger.warning(
                f"Trade log file corrupted, skipping pending trades display: {e}"
            )
        ...
        if no_signals_streak > 0:
            print(
                f"[ALERT] No-Signals Streak: {no_signals_streak} (triggers at 2, max was {max_no_signals})"
            )
        else:
        ...
        print(
            f"  • Confidence Threshold: {params.get('confidence_threshold', 0.3):.2f}"
        )
        print(
            f"  • Entry Adjustment: {params.get('entry_adjustment_factor', 1.0):.2f}x"
        )
        print(
            f"  • Stop Loss Adjustment: {params.get('sl_adjustment_factor', 1.0):.2f}x"
        )
        print(
            f"  • Take Profit Adjustment: {params.get('tp_adjustment_factor', 1.0):.2f}x"
        )
...
def main():
    ...
    print(
        f"[TARGET] Strategy: News/AI Primary (85-90%) + Candlestick Analysis (10-15%)"
    )
    print(f"[ANALYSIS] Candlestick Patterns: FREE (TA-Lib, no API calls)")
    try:
        dyn_lev = (
            market_analyzer.get_adjusted_parameters().get(
                "dynamic_max_leverage", MAX_LEVERAGE_CRYPTO
            )
            if market_analyzer
            else MAX_LEVERAGE_CRYPTO
        )
    except Exception:
    ...
    print(
        f"[RISK] Stop Loss: {MIN_STOP_PCT * 100:.1f}%-{MAX_STOP_PCT * 100:.1f}%, Take Profit: up to {MAX_TP_PCT * 100:.1f}%"
    )
    ...
    print(
        f"[CACHE] News Cache: {cache_stats['total_cached']} analyzed, resets in {cache_stats['will_reset_in_hours']:.1f}h"
    )
    print("=" * 70)
    print()
    ...
    print(
        f"[ANALYZE] Analyzing {len(symbol_articles)} cryptocurrencies: {list(symbol_articles.keys())}\n"
    )
    ...
    if signals:
        ...
        for i, item in enumerate(signals[:3]):  # Show first 3 signals
            signal = item["signal"]
            print(
                f"  {i + 1}. {item['symbol']}: confidence={signal.get('confidence', 0):.3f}, direction={signal.get('direction', 'N/A')}"
            )
    else:
        ...
        try:
            ...
            if market_data:
                print(
                    f"[DEBUG] Market data OK for {test_symbol}: ${market_data['price']:.2f}"
                )
                ...
                if articles_for_symbol:
                    ...
                    if sentiment_result:
                        ...
                        test_signal = calculate_trade_signal(
                            sentiment_score,
                            len(articles_for_symbol),
                            market_data,
                            test_symbol,
                        )
                        if test_signal:
                            print(
                                f"[DEBUG] Signal generated: {test_signal['direction']} with confidence {test_signal['confidence']:.3f}"
                            )
                        else:
    ...
    if not signals:
        ...
        if market_analyzer:
            ...
            params = market_analyzer.get_adjusted_parameters()
            no_signals_count = market_analyzer.precision_metrics.get(
            ...
            if no_signals_count >= 2:
                ...
                print(
                    f"  • SL Adjustment: {params.get('sl_adjustment_factor', 1.0):.2f}x"
                )
                print(
                    f"  • TP Adjustment: {params.get('tp_adjustment_factor', 1.0):.2f}x"
                )
            else:
                print(
                    f"[LEARN] Will loosen after {2 - no_signals_count} more consecutive no-signal run(s)"
                )
...
[TARGET] {len(signals)} Signals Found\n"""
    ...
    for idx, item in enumerate(signals[:5], start=1):  # Top 5 signals
        msg = format_trade_message(
            item["symbol"], item["signal"], item["sentiment_reason"], signal_number=idx
        )
        print(msg)
        print()
    ...
    try:
        ...
        print(
            f"[DEBUG] Successfully logged {len(logged_trades)} total trades to {TRADE_LOG_FILE}"
        )
        if logged_trades:
            latest_trade = logged_trades[-1]
            print(
                f"[DEBUG] Latest trade: {latest_trade['symbol']} {latest_trade['direction']} (status: {latest_trade['status']})"
            )
    except FileNotFoundError:
