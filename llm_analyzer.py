...
TIMEFRAME: [HOURS/DAYS/WEEK]"""
    ...
    def track_signals_generated(self):
        ...
        This resets the consecutive no-signals counter and gradually tightens
        parameters back toward baseline if they were previously loosened.
        """
        # Reset consecutive no-signals counter
        self.precision_metrics['consecutive_no_signals'] = 0
        
        # GRADUALLY tighten parameters back toward baseline if they were loosened
        # This provides automatic recovery when the market conditions improve
        tightened_any = False
        
        # 1. Gradually restore entry adjustment toward 1.0 (baseline)
        current_entry = self.strategy_adjustments['entry_adjustment_factor']
        if current_entry < 1.0:  # Only tighten if it was loosened below baseline
            # Gradual restoration: move 10% of the way back to baseline each time
            gap_to_baseline = 1.0 - current_entry
            restoration_amount = gap_to_baseline * 0.1  # 10% restoration
            new_entry = min(1.0, current_entry + restoration_amount)
            
            if new_entry != current_entry:
                self.strategy_adjustments['entry_adjustment_factor'] = new_entry
                tightened_any = True
                print(f"🔧 Signals generated - gradually tightening entry adjustment: {current_entry:.2f}x → {new_entry:.2f}x")
        
        # 2. Gradually restore confidence threshold toward baseline (0.3)
        current_conf = self.strategy_adjustments['confidence_threshold']
        baseline_conf = 0.3
        if current_conf < baseline_conf:  # Only tighten if it was lowered
            gap_to_baseline = baseline_conf - current_conf
            restoration_amount = gap_to_baseline * 0.1  # 10% restoration
            new_conf = min(baseline_conf, current_conf + restoration_amount)
            
            if new_conf != current_conf:
                self.strategy_adjustments['confidence_threshold'] = new_conf
                tightened_any = True
                print(f"🔧 Signals generated - gradually tightening confidence threshold: {current_conf:.2f} → {new_conf:.2f}")
        
        # 3. Gradually restore SL adjustment toward 1.0 (baseline)
        current_sl = self.strategy_adjustments['sl_adjustment_factor']
        if current_sl > 1.0:  # Only tighten if it was widened above baseline
            gap_to_baseline = current_sl - 1.0
            restoration_amount = gap_to_baseline * 0.1  # 10% restoration
            new_sl = max(1.0, current_sl - restoration_amount)
            
            if new_sl != current_sl:
                self.strategy_adjustments['sl_adjustment_factor'] = new_sl
                tightened_any = True
                print(f"🔧 Signals generated - gradually tightening SL adjustment: {current_sl:.2f}x → {new_sl:.2f}x")
        
        # 4. Gradually restore TP adjustment toward 1.0 (baseline)
        current_tp = self.strategy_adjustments['tp_adjustment_factor']
        if current_tp < 1.0:  # Only tighten if it was reduced below baseline
            gap_to_baseline = 1.0 - current_tp
            restoration_amount = gap_to_baseline * 0.1  # 10% restoration
            new_tp = min(1.0, current_tp + restoration_amount)
            
            if new_tp != current_tp:
                self.strategy_adjustments['tp_adjustment_factor'] = new_tp
                tightened_any = True
                print(f"🔧 Signals generated - gradually tightening TP adjustment: {current_tp:.2f}x → {new_tp:.2f}x")
        
        if tightened_any:
            print("✅ Parameters gradually tightened back toward baseline")
        else:
            print("✅ Signals generated - consecutive no-signals counter reset")
